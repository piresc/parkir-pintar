...
func TestProperty5_DifferentIdempotencyKeysProduceDifferentIDs(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		createForKey := func(key string) (*model.Reservation, error) {
			...
			billing.On("StartBilling", mock.Anything, mock.AnythingOfType("string"), pricing.BookingFee, mock.AnythingOfType("string")).Return(&reservation.BillingRecord{ID: "billing-test-id"}, nil)
...
func TestProperty9_ReservationCreationPostconditions(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		billing.On("StartBilling", mock.Anything, mock.AnythingOfType("string"), pricing.BookingFee, mock.AnythingOfType("string")).Return(&reservation.BillingRecord{ID: "billing-prop9-id"}, nil)
...
func TestProperty10_NoDoubleBooking(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		billing1.On("StartBilling", mock.Anything, mock.AnythingOfType("string"), pricing.BookingFee, mock.AnythingOfType("string")).Return(&reservation.BillingRecord{ID: "billing-first-id"}, nil)
		...
		uc2 := NewUsecase(repo2, locker2, billing2, payment2, nil, nil, nil, 60, 10)
