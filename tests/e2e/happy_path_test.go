...
// complete reservation lifecycle: create → check-in → check-out with system
...
func TestHappyPath_ShouldCompleteFullLifecycle_WhenSystemAssigned(t *testing.T) {
	...
	// Assert — Step 1: Reservation confirmed, spot reserved
	require.NoError(t, err)
	...
	// Assert — Step 2: Billing record created with booking_fee=5000
	...
	// Assert — Step 3: CHECKED_IN, spot occupied
	require.NoError(t, err)
	...
	// Act — Step 4: Check out
	checkoutResp, err := env.reservationUC.CheckOut(ctx, &model.CheckOutRequest{
		ReservationID: reservation.ID,
	})

	// Assert — Step 4: CHECKED_OUT, payment processed, spot available
	require.NoError(t, err)
